...
int main(int argc,char **argv)
{
   ...
   for(j=0;j<34;j++)
	...
	//creates a 32x32 board of random spin values for each array box
   ...
   for(j=1;j<=32;j++)
	...
	fp=fopen("Magnetism Values 10000_2.txt", "w+");
	...
	for(times=0;times<=10000;times++)
	{
        for(i=1;i<=32;i++)//Randomly selects 32 cells
		{
            ...
            r=RandomUniform();
			j=(int)((r*32)+1);//max r value ~.99 Need 1-32 as range
			r=RandomUniform();
			k=(int)((r*32)+1);
			...
			//finds delta H
			h=(B[j][k])*(B[j+1][k]+B[j-1][k]+B[j][k+1]+B[j][k-1]);
		...
		mag=0;
		for(j=1;j<=32;j++)//finds the resulting magnetism of the whole array
		{
            for(k=1;k<=32;k++)
			{
				mag=mag+B[j][k];
