#include "stdio.h"
#include "stdlib.h"
#include "math.h"
void   RandomInitialise(int,int);
double RandomUniform(void);
double RandomGaussian(double,double);
int    RandomInt(int,int);
double RandomDouble(double,double);

int plus ( int num1 );
int minus ( int num2 );


int main(int argc,char **argv)
{

	int i, j, k, z, repeat, occ, endpt, check;
	double r,rmin=1e32,rmax=-1e32;
	int B[40][40], x0_place[16], y0_place[16], x_place[1600], y_place[1600];
	FILE *fp;

	

	/* Generate 20000 random numbers */
	RandomInitialise(1802,9373);
	for (i=0;i<20000;i++) 
	{
		r = RandomUniform();
		if (r < rmin) rmin = r;
		if (r > rmax) rmax = r;
	}
	fprintf(stderr,"Numbers range from %g to %g\n",rmin,rmax);

	//pre-sets everything to 0
	for(j=0;j<=39;j++)
	{
		for(k=0;k<=39;k++)
		{
			B[j][k]=0;
		}
	}
	
	//starting point will be (arbitrarily) at B[20][20].
	B[20][20]=1;
	j=20;
	k=20;
	y0_place[0]=j;
	x0_place[0]=k;

	for(i=1;i<=15;i++)//creates the random walk of length 16
	{
		r=(int)(RandomUniform()*4);

		if(r==0)//next point is one coordinate up
		{
			j=plus(j);
			B[j][k]=B[j][k]++;//adding the point on the grid
			
			//updating the coordinates:
			y0_place[i]=j;
			x0_place[i]=k;
		}
		if(r==1)//next point is one coordinate down
		{
			j=minus(j);
			B[j][k]=B[j][k]++;
			y0_place[i]=j;
			x0_place[i]=k;
		}
		if(r==2)//next point is one coordinate to the right
		{
			k=plus(k);
			B[j][k]=B[j][k]++;
			y0_place[i]=j;
			x0_place[i]=k;
		}
		if(r==3)//next point is one coordinate left
		{
			k=minus(k);
			B[j][k]=B[j][k]++;
			y0_place[i]=j;
			x0_place[i]=k;
		}
	}

	fp=fopen("Self Avoid Snake.txt", "w");
	fprintf(fp, "Original Walk\n");
	fprintf(fp, "x y\n");
	for(i=0;i<=15;i++)
	{
		fprintf(fp, "%d %d\n", x0_place[i], y0_place[i]);
	}


	//The deleting and adding  part:

	repeat=1;
	while(repeat==1)//will repeat as long as repeat==1
	{
		//picks to either delete the end pt or the beginning pt
		endpt=(int)(RandomUniform()*2);

		//deletes pt at the end -> adds to beginning
		if(endpt==0)
		{
			//copies the all the coordinates except last one on to another array
			for(z=1; z<=15; z++)//total 0-15
			{
				y_place[z]/*1-15*/=y0_place[z-1]/*0-14*/;
				x_place[z]=x0_place[z-1];
			}

			//removes end point from grid...the update
			B[y0_place[15]][x0_place[15]]=B[y0_place[15]][x0_place[15]]--;

			//determines which spaces are occupied at beginning point
			if(B[y0_place[0]][minus(x0_place[0])]!=0)//left space is occupied
			{
				occ=0;
			}
			if(B[minus(y0_place[0])][x0_place[0]]!=0)//top space is occupied
			{
				occ=1;
			}
			if(B[y0_place[0]][plus(x0_place[0])]!=0)//right space is occupied
			{
				occ=2;
			}
			if(B[plus(y0_place[0])][x0_place[0]]!=0)//bottom space is occupied
			{
				occ=3;
			}

			//attempts to add to the beginning of the random walk. If space is occupied, retries.
			while(repeat==1)
			{
				r=(int)(RandomUniform()*4);//picks which side to add at beginning

				if(r==0 && occ!=0)//adds to the left side
				{
					y_place[0]=y0_place[0];
					x_place[0]=minus(x0_place[0]);
					repeat=0;//repeat is change to 0 since adding new pt is successful
				}
				if(r==1 && occ!=1)//adds to the top side
				{
					y_place[0]=minus(y0_place[0]);
					x_place[0]=x0_place[0];
					repeat=0;
				}
				if(r==2 && occ!=2)//adds to the right side
				{
					y_place[0]=y0_place[0];
					x_place[0]=plus(x0_place[0]);
					repeat=0;
				}
				if(r==3 && occ!=3)//adds to the bottom side
				{
					y_place[0]=plus(y0_place[0]);
					x_place[0]=x0_place[0];
					repeat=0;
				}
			}
            //y_place and x_place now have the most updated coordinates of walk

			//updating grid
			B[y_place[0]][x_place[0]]=B[y_place[0]][x_place[0]]++;

            //transfering updated coordinates from dummy array to the original array

			for(z=0;z<=15;z++)
			{
				y0_place[z]=y_place[z];
				x0_place[z]=x_place[z];
			}
		}
		if(endpt==1)//deletes point at the beginning -> adds to end
		{
			for(z=0; z<=14; z++)//transfering coordinates to a dummy array
			{
				y_place[z]/*0-14*/=y0_place[z+1]/*1-15*/;
				x_place[z]=x0_place[z+1];
			}

			//removes initial point from grid...the update
			B[y0_place[0]][x0_place[0]]=B[y0_place[0]][x0_place[0]]--;

			//determines which spaces are occupied at last point
			if(B[y0_place[15]][minus(x0_place[15])]!=0)
			{
				occ=0;
			}
			if(B[minus(y0_place[15])][x0_place[15]]!=0)
			{
				occ=1;
			}
			if(B[y0_place[15]][plus(x0_place[15])]!=0)
			{
				occ=2;
			}
			if(B[plus(y0_place[15])][x0_place[15]]!=0)
			{
				occ=3;
			}

			while(repeat==1)
			{
				r=(int)(RandomUniform()*4);

				if(r==0 && occ!=0)
				{
					y_place[15]=y0_place[15];
					x_place[15]=minus(x0_place[15]);
					repeat=0;
				}
				if(r==1 && occ!=1)
				{
					y_place[15]=minus(y0_place[15]);
					x_place[15]=x0_place[15];
					repeat=0;
				}
				if(r==2 && occ!=2)
				{
					y_place[15]=y0_place[15];
					x_place[15]=plus(x0_place[15]);
					repeat=0;
				}
				if(r==3 && occ!=3)
				{
					y_place[15]=plus(y0_place[15]);
					x_place[15]=x0_place[15];
					repeat=0;
				}
			}

			//y_place and x_place now have the most updated coordinates of walk

			//updating grid
			B[y_place[15]][x_place[15]]=B[y_place[15]][x_place[15]]++;

            //transfering updated coordinates from dummy array to the original array
			for(z=0;z<=15;z++)
			{
				y0_place[z]=y_place[z];
				x0_place[z]=x_place[z];
			}
		}

		//checks for self avoiding.
		repeat=1;//ensures the while loop will repeat, will be negated if walk is self avoiding
		check=0;//will remain 0 if walk is self avoiding
		
		for(j=0;j<=39;j++)
		{
			for(k=0;k<=39;k++)
			{
				if(B[j][k]>1)//i.e. walk is not self avoiding
				{
					check=check++;//check is changed
				}
			}
		}
		if(check==0)//negates the repeat
		{
			repeat=0;
		}

	}

	//prints out the final grid
	for(j=0;j<=39;j++)
	{
		for(k=0;k<=39;k++)
		{
			printf("%d ", B[j][k]);
		}

		printf("\n");
	}


	//prints coordinates of final random walk
	fprintf(fp, "Self Avoiding Walk\n");
	fprintf(fp, "\nx y\n");
	for(i=0;i<=15;i++)
	{
		fprintf(fp, "%d %d\n", x0_place[i], y0_place[i]);
	}
	fclose(fp);
}

int plus ( int num1 )
{
	int movement1;
	movement1=((num1+1) + (5*40))%40;

	return movement1;
}

int minus ( int num2 )
{
	int movement2;
	movement2=((num2-1) + (5*40))%40;

	return movement2;
}

int minus1( int num2 );
#define FALSE 0
#define TRUE 1
/* Globals */
double u[97],c,cd,cm;
int i97,j97;
int test = FALSE;

void RandomInitialise(int ij,int kl)
{
   double s,t;
   int ii,i,j,k,l,jj,m;

   if (ij < 0 || ij > 31328 || kl < 0 || kl > 30081) {
		ij = 1802;
		kl = 9373;
   }

   i = (ij / 177) % 177 + 2;
   j = (ij % 177)       + 2;
   k = (kl / 169) % 178 + 1;
   l = (kl % 169);

   for (ii=0; ii<97; ii++) {
      s = 0.0;
      t = 0.5;
      for (jj=0; jj<24; jj++) {
         m = (((i * j) % 179) * k) % 179;
         i = j;
         j = k;
         k = m;
         l = (53 * l + 1) % 169;
         if (((l * m % 64)) >= 32)
            s += t;
         t *= 0.5;
      }
      u[ii] = s;
   }

   c    = 362436.0 / 16777216.0;
   cd   = 7654321.0 / 16777216.0;
   cm   = 16777213.0 / 16777216.0;
   i97  = 97;
   j97  = 33;
   test = TRUE;
}

double RandomUniform(void)
{
   double uni;

   if (!test) 
   	RandomInitialise(1802,9373);

   uni = u[i97-1] - u[j97-1];
   if (uni <= 0.0)
      uni++;
   u[i97-1] = uni;
   i97--;
   if (i97 == 0)
      i97 = 97;
   j97--;
   if (j97 == 0)
      j97 = 97;
   c -= cd;
   if (c < 0.0)
      c += cm;
   uni -= c;
   if (uni < 0.0)
      uni++;

   return(uni);
}

double RandomGaussian(double mean,double stddev)
{
   double  q,u,v,x,y;

   do {
      u = RandomUniform();
      v = RandomUniform();
   	if (u <= 0.0 || v <= 0.0) {
       	u = 1.0;
       	v = 1.0;
   	}
      v = 1.7156 * (v - 0.5);

      /*  Evaluate the quadratic form */
      x = u - 0.449871;
   	y = fabs(v) + 0.386595;
      q = x * x + y * (0.19600 * y - 0.25472 * x);

      /* Accept P if inside inner ellipse */
      if (q < 0.27597)
			break;

      /*  Reject P if outside outer ellipse, or outside acceptance
 region */
    } while ((q > 0.27846) || (v * v > -4.0 * log(u) * u * u));

    /*  Return ratio of P's coordinates as the normal deviate */
    return (mean + stddev * v / u);
}

/*
   Return random integer within a range, lower -> upper INCLUSIVE
*/
int RandomInt(int lower,int upper)
{
   return((int)(RandomUniform() * (upper - lower + 1)) + lower);
}

/*
   Return random float within a range, lower -> upper
*/
double RandomDouble(double lower,double upper)
{
   return((upper - lower) * RandomUniform() + lower);
}